...
if [ $(whoami) = "root" ]; then
     ...
     log "Install nonfree and multilib repo"
     sudo xbps-install -Sy \
     ...
     log "Install deps for FFMPEG 6.1"
     sudo xbps-install -Sy \
          perl \
          yasm \
          alsa-lib-devel \
     ...
     check "$?" "Install deps for FFMPEG 6.1"
     log "Deps for FFMPEG 6.1 installed"

     log "Building FFMPEG 6.1"
     wget https://ffmpeg.org/releases/ffmpeg-6.1.tar.gz
     check "$?" "WGET FFMPEG"
     tar -xvzf ffmpeg-6.1.tar.gz
     check "$?" "TAR FFMPEG"
     cd ffmpeg-6.1
     ./configure \
     --prefix='/usr' \
